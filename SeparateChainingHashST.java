...
/*
 * This is a modified version of the code provided by the authors as mentioned below. 
 * I've implemented the Horner's method to make hashing of the strings more efficient and 
 * sparse.
 */
 ...
 * <p>
 * This implementation uses a separate chaining hash table. It requires that the
...
public class SeparateChainingHashST<Key, Value> {
	private static final int INIT_CAPACITY = 859; // A Prime Number

	private int n; // number of key-value pairs
	private int m; // hash table size
	private SequentialSearchST<String, Value>[] st; // array of linked-list
													// symbol tables
	...
	public SeparateChainingHashST(int m) {
		this.m = m;
		st = (SequentialSearchST<String, Value>[]) new SequentialSearchST[m];
		for (int i = 0; i < m; i++)
			st[i] = new SequentialSearchST<String, Value>();
	}

	// resize the hash table to have the given number of chains,
	// rehashing all of the keys
	private void resize(int chains) {
		SeparateChainingHashST<String, Value> temp = new SeparateChainingHashST<String, Value>(chains);
		for (int i = 0; i < m; i++) {
	...
	private int hash(String key) {
		return (horner(key) & 0x7fffffff) % m;
	}
